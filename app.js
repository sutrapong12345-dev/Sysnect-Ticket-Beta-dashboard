...
async function fetchDbSyncTime() {
    const el      = document.getElementById('dbSyncText');
    const iconEl  = document.getElementById('dbSyncIcon');
    const badgeEl = document.getElementById('dbSyncBadge');
    if (!el) return;

    const setStatus = (label, color, icon, timeLabel) => {
        el.innerText = `DB: ${label}${timeLabel ? ' · ' + timeLabel : ''}`;
        if (iconEl)  { iconEl.innerText = icon; iconEl.style.color = color; }
        if (badgeEl) {
            badgeEl.style.background  = hexToRgba(color, 0.07);
            badgeEl.style.borderColor = hexToRgba(color, 0.30);
        }
    };
    ...
    try {
        ...
        const syncResult = json?.last_sync_result;
        const dbInfo     = json?.database;
        const rawTime    = syncResult?.at || dbInfo?.sync_state?.last_sync;
        let timeLabel = '';
        if (rawTime) {
            const d = new Date(rawTime);
            timeLabel = d.toLocaleDateString('th-TH', { day: '2-digit', month: 'short' })
                      + ' ' + d.toLocaleTimeString('th-TH', { hour: '2-digit', minute: '2-digit' });
        }

        if (syncResult?.ok && syncResult?.source === 'n8n') {
            setStatus('จาก n8n', '#10b981', 'cloud_sync', timeLabel);
        } else if (syncResult?.ok && syncResult?.source === 'glpi_direct') {
            setStatus('จาก API GLPI', '#f59e0b', 'api', timeLabel);
        } else if (dbInfo?.connected && (dbInfo?.counts?.total ?? 0) > 0) {
            setStatus('Database', '#3b82f6', 'database', timeLabel);
        } else {
            setStatus('ไม่พร้อมใช้', '#ef4444', 'error_outline', '');
        }
    } catch (_) {
        setStatus('ไม่พร้อมใช้', '#ef4444', 'error_outline', '');
    }
...
async function fetchData() {
    ...
    try {
        ...
        // 🆕 หลัก = Node API ที่อ่านจาก PostgreSQL (เร็ว <50ms, สะสมประวัติ)
        //    สำรอง = n8n webhook ตรง (เผื่อเปิดหน้าเว็บแบบไม่ได้เสิร์ฟผ่าน Node server)
        const API_URL = (location.protocol.startsWith('http'))
            ? `${location.origin}/api/tickets`
            : 'http://localhost:3000/api/tickets';
        const FALLBACK_URL = 'https://n8n.sysnect.co.th/webhook/48ec49ee-a4ca-4677-bad7-deb3c3ec341d';

        let liveData = null;
        STATE.isFallback = false;

        try {
            // ดึงจาก Node API (PostgreSQL)
            const loaderMessage = document.getElementById('loaderMessage');
            if (loaderMessage) loaderMessage.innerText = "กำลังดึงข้อมูลจากฐานข้อมูล...";
            liveData = await fetchWithTimeout(API_URL, 20000);
        } catch (apiError) {
            console.warn("Node API failed, attempting n8n direct...", apiError);
            const loaderMessage = document.getElementById('loaderMessage');
            if (loaderMessage) loaderMessage.innerText = "ฐานข้อมูลไม่พร้อม ตอนนี้กำลังดึงข้อมูลตรงจาก n8n...";

            try {
                liveData = await fetchWithTimeout(FALLBACK_URL, 45000);
                STATE.isFallback = true;
            } catch (fallbackError) {
                console.error("Fallback failed...", fallbackError);
                const loaderMsg = document.getElementById('loaderMessage');
                if (loaderMsg) {
                    loaderMsg.innerText = "ไม่สามารถเชื่อมต่อกับเซิร์ฟเวอร์หลักและสำรองได้";
