...
// อัปเดตไฟสถานะ 2 ดวง: n8n + Redis Cache
//   n8n  : เขียวถ้าข้อมูลรอบนี้มาจาก n8n (ผ่าน Node หรือยิงตรง)
//   PG   : เขียวถ้า /api/health บอกว่า DB connected หรือ source=='redis'
async function updateConnectionStatus() {
    ...
    // ── Redis Cache (ถาม /api/health) ──
    let pgUp = (source === 'redis');
    let pgTimeLabel = '';
    try {
    ...
    if (pgText) pgText.innerText = pgUp
        ? ('Redis Cache' + (pgTimeLabel ? ' · ' + pgTimeLabel : ''))
        : 'Redis Cache ล่ม';
}
...
async function fetchData() {
    ...
    try {
        ...
        //    หลัก = Node API /api/tickets ซึ่งจะลอง n8n ก่อน (พร้อม upsert ลง Redis ให้)
        //           ถ้า n8n ล่ม Node จะส่งข้อมูลจาก Redis แทน (_meta.source='redis')
        ...
        try {
            ...
            if (STATE.dataSource === 'redis') {
                STATE.isFallback = true; // n8n ล่ม → ใช้ข้อมูลล่าสุดจาก Redis
            ...
            try {
                ...
                if (loaderMsg) {
                    loaderMsg.innerText = "ฐานข้อมูลไม่พร้อม — เชื่อมต่อ n8n และ Redis Cache ไม่ได้";
        ...
        updateConnectionStatus(); // ไฟสถานะแดงทั้ง n8n + Redis Cache
